...
cities.df <- select(us.cities,name, country.etc, pop, long)
...
#clean up colnames
colnames(cities.df) <- c("city", "state", "pop", "long")
colnames(avocado.df) <- c("city", "avg.price")

#join tables
avocado.df <- inner_join(avocado.df, cities.df, by="city")

ggplot(avocado.df, aes(x=long, y=avg.price)) +
  geom_point(color="#6a403a", size=3) +
  ggtitle("Avocado Prices in the United States by Longitude") +
  xlab("Longitude (°)") +
